...
std::vector<Light> GuideLightDetector::findLights(const cv::Mat & rgb_img, const cv::Mat & binary_img) {
    ...
    if (l_.use_nn) {
        candidates = findLightsByNN(rgb_img);
        if (candidates.empty() && l_.nn_fallback_to_traditional) {
            candidates = findLightsByTraditional(rgb_img, binary_img);
        }
    } else {
        candidates = findLightsByTraditional(rgb_img, binary_img);
    }

    return smoothCandidates(candidates);
}

std::vector<Light> GuideLightDetector::findLightsByTraditional(
    const cv::Mat & rgb_img, const cv::Mat & binary_img) {
    (void)rgb_img;
    std::vector<Light> candidates;
