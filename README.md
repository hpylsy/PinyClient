...
- UDP 图传监听：`0.0.0.0:3334`，即监听本机所有网卡。
- 图传源策略：`auto`。英雄根据 `DeployModeStatusSync.status` 自动在 UDP/MQTT 图传之间切换；非英雄固定 UDP。
...
# 如果必须只监听有线网卡 192.168.12.2
python3 app.py --udp-bind-host 192.168.12.2

# 临时强制 MQTT 图传源，仅用于调试
python3 app.py --video-source mqtt

# 临时强制 UDP 图传源，仅用于调试
python3 app.py --video-source udp

# 启动交互命令行
python3 app.py --console
```
...
### 测试说明
- 不要通过修改 `app.py` 源码来切换测试模式。
- 单独测试 PinyClient 图传源时，使用 `python3 app.py --video-source mqtt/udp/none`。
- 完整本机端到端图传模拟使用 `tools/local_mqtt_video_sim.py`，详见 [tools/README.md](tools/README.md)。
